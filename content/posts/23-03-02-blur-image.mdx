---
title: "Image blur loading"
summary: "In this tutorial, we will add blur effect to a div with the background image step by step"
date: 2023-03-2
image:
...
cause I have seen a similar effect in Nextjs `<Image>{:js}`.
...
Now, let's build this `<ImageBg>{:js}` with blur effect step by step!

<Tips title="Pre-requisite">
...
Then let's build a `<div>{:js}` with a simple image background in `ImageBg.vue`.
...
Blurhash provides an algorithm to convert an image to a <b>Blurhash string</b>, 
...
Now, please upload one image you like and keep the <b>Blurhash string</b>.
For me, it is:
