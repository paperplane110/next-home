...
export function OdysseySidebar() {
  ...
  const allExpanded = openCategories.size === ODYSSEY_CATEGORIES.length;

  const toggleAllCategories = () => {
    setOpenCategories(
      allExpanded ? new Set<OdysseyCategory>() : new Set(ODYSSEY_CATEGORIES)
    );
  };

  return (
    <div className="w-full h-full min-h-0 flex flex-col" data-lenis-prevent data-lenis-prevent-wheel>
      <div id="sidebar-title" className="z-20 flex items-center gap-1">
        <Link className="rounded-md flex-1 min-w-0" href={getOdysseyHomeHref(locale)}>
          <div className={cn(
        ...
        <button
          ...
          aria-label={allExpanded ? copy.collapseAllCategories : copy.expandAllCategories}
          title={allExpanded ? copy.collapseAllCategories : copy.expandAllCategories}
          className={cn(
            "mb-2 shrink-0 p-2 rounded-md text-neutral-400",
            "transition-colors hover:bg-neutral-100/80 hover:text-neutral-700"
          )}
        >
          <span className="relative block size-4">
            <ChevronsUpDown
              className={cn(
                "absolute inset-0 size-4 transition-[opacity,filter] duration-200",
                "ease-[cubic-bezier(0.645,0.045,0.355,1)] motion-reduce:transition-none",
                allExpanded ? "opacity-0 blur-[2px]" : "opacity-100 blur-none"
              )}
            />
            <ChevronsDownUp
              className={cn(
                "absolute inset-0 size-4 transition-[opacity,filter] duration-200",
                "ease-[cubic-bezier(0.645,0.045,0.355,1)] motion-reduce:transition-none",
                allExpanded ? "opacity-100 blur-none" : "opacity-0 blur-[2px]"
              )}
            />
          </span>
        </button>
      </div>
      <ScrollArea className="relative -mt-4 flex-1 h-0 min-h-0 w-full **:data-[slot=scroll-area-scrollbar]:hidden">
